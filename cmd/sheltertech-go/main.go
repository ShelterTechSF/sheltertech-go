...
func main() {
	...
	searchService := algolia.NewSearchService(algoliaAppID, algoliaAPIKey, algoliaIndexPrefix, dbManager)
	categoriesManager := categories.New(dbManager)
	changeRequestManager := changerequest.New(dbManager, searchService)
	...
	r.Group(func(r chi.Router) {
		...
		r.Patch("/api/change_requests/{id}/approve", changeRequestManager.Approve)
	})
